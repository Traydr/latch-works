...
- Scan ordering (2026-09-25): scans run one at a time. `ScanQueue` in the main process serializes `scan:start`; a request made during a scan cancels it (the worker's existing cancel path) and starts once the worker reports it idle (`wait-for-scan`), only the newest of the waiting requests runs, and the superseded scan's events are dropped before they reach the renderer. The renderer's `runScan` also stops listening to the running scan as it asks for a new one. Finishing the old scan first was tried and rejected: navigation during a big scan waited for the whole scan. Folders the user chose (Open dialog, drop, OS open) get a claimable grant (`grantChosenMediaRoot`/`claimChosenMediaRoot` in `mediaProtocol.ts`) that survives another request's root shrink until their own scan claims it; only the 4 most recent unclaimed grants are kept. Every failed scan request that is still the newest sends a scan `error` event so the renderer never stays in "Scanning". Upstream of the queue, OS opens are processed in arrival order (`openLaunchPath` chains them in `main.ts`) and the renderer's `scanInputPathAction` scans only the newest of paths still resolving, so back-to-back opens cannot reach the queue out of order.
