...
import type { ViewerVideoModel } from '../../../hooks/useViewerVideoModel';
...
/** A chrome surface: clicks inside never reach the picture's click handler. */
export function ChromeRegion({
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: a propagation fence, not a control
 ...
 * when the pointer leaves the track, and `touch-action: none` stops a touch
 * screen from scrolling instead of scrubbing.
 */
...
/** Seek state and gestures against the model's video: tap to jump, drag to scrub. */
export function useSeek(model: ViewerVideoModel): SeekControl {
  const { canSeek, duration, position } = model;
  const frameRef = useRef<number | null>(null);
  const pendingRef = useRef<number | null>(null);

  const previewSeek = (fraction: number): void => {
  ...
  const commit = (fraction: number): void => {
    ...
    model.commitSeek(fraction * duration);
  };
 ...
 * finger lifts. Any part of the picture works; the arrows and the chrome sit
 * above the picture and take their own presses. Mouse presses are ignored:
 * a click plays or pauses instead.
 */
export function useHoldToBoost(model: ViewerVideoModel): HoldToBoost {
  const timerRef = useRef<number | null>(null);
  const suppressClickRef = useRef(false);
  const { beginHoldBoost, endHoldBoost, holdBoosting, playing } = model;
