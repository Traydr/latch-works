...
export interface VideoPlayerChromeProps extends ModelProps {
 ...
 * from the capsule; narrow windows fold both behind one cog. Title, file
 * actions, fullscreen, close and prev/next stay in the viewer's shared chrome.
 */
export function VideoPlayerChrome({
  chromeVisibilityClass,
  model,
}: VideoPlayerChromeProps): JSX.Element {
...
function Popup({
  ...
  return (
    // Narrow windows anchor panels to the capsule's top edge; wider ones centre them on the trigger.
...
/** The narrow-window form of the volume control: speaker, level bar and percentage in one row. */
function VolumeRow({ model }: ModelProps): JSX.Element {
  const level = model.muted ? 0 : model.volume;
...
function SpeedPanel({ model, onPick }: ModelProps & { onPick: () => void }): JSX.Element {
  return (
...
/** The narrow-window form of the speed picker: one row of pills. */
function SpeedRow({ model }: ModelProps): JSX.Element {
  return (
...
/** Narrow windows only: volume and speed behind the cog. */
function SettingsPanel({ model }: ModelProps): JSX.Element {
  return (
